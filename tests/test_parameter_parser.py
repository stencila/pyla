...
def test_parameter_deserializer():
    ...
    assert (
        pp.deserialize_parameter(
            Parameter("p", validator=ConstantValidator(value="abc123")), "some string"
        )
        == "abc123"
    )
    assert (
        pp.deserialize_parameter(
            Parameter("p", validator=ConstantValidator(value="abc123")), None
        )
        == "abc123"
    )
    ...
    assert (
        pp.deserialize_parameter(
            Parameter("p", validator=EnumValidator(values=["a", "b"])), "a"
        )
        == "a"
    )
    ...
    with pytest.raises(TypeError):
        ...
        pp.deserialize_parameter(
            Parameter("p", validator=EnumValidator(values=["a", "b"])), "c"
        )

    for t in ["TRUE", "true", "t", "T", "1", "YES", "yes"]:
        assert (
            pp.deserialize_parameter(Parameter("p", validator=BooleanValidator()), t)
            is True
        )

    for f in ["false", "0", "no", "", "anything else"]:
        assert (
            pp.deserialize_parameter(Parameter("p", validator=BooleanValidator()), f)
            is False
        )

    assert (
        pp.deserialize_parameter(Parameter("p", validator=IntegerValidator()), "1000")
        == 1000
    )

    # Not pi. But close.
    assert (
        pp.deserialize_parameter(Parameter("p", validator=NumberValidator()), "3.1418")
        == 3.1418
    )

    assert (
        pp.deserialize_parameter(Parameter("p", validator=StringValidator()), "3.1418")
        == "3.1418"
    )

    assert pp.deserialize_parameter(
        Parameter("p", validator=ArrayValidator()), "[5, 6, 7]"
    ) == [5, 6, 7]

    assert pp.deserialize_parameter(
        Parameter("p", validator=TupleValidator()), '[1, true, "Up"]'
    ) == [1, True, "Up"]
